...
# 2. Create components/FloatingContact.tsx
cat << 'EOF' > components/FloatingContact.tsx
'use client';
    ...
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-center">
      <div className={`flex flex-col gap-3 mb-4 transition-all duration-300 origin-bottom ${isOpen ? 'scale-100 opacity-100 visible' : 'scale-75 opacity-0 invisible pointer-events-none'}`}>
        ...
        <a href="https://zalo.me/0364518980" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-12 h-12 bg-[#0068ff] text-white rounded-full shadow-lg hover:scale-110 transition-transform">
          <svg viewBox="0 0 48 48" fill="none" className="w-7 h-7"><path d="M24 9C14.06 9 6 16.03 6 24.7c0 4.75 2.37 8.98 6.13 11.83-1.04 3.03-3.21 6.36-3.21 6.36-.2.27.04.59.34.52 1.45-.33 5.42-1.74 8.78-3.4 1.88.46 3.88.72 5.96.72 9.94 0 18-7.03 18-15.73C42 16.03 33.94 9 24 9z" fill="white"/><text x="24" y="29.5" fill="#0068ff" fontSize="13" fontWeight="900" fontFamily="Arial" textAnchor="middle">Zalo</text></svg>
        </a>
      ...
      <button 
        ...
        className="flex items-center justify-center w-14 h-14 bg-blue-600 text-white rounded-full shadow-xl hover:bg-blue-700 hover:scale-110 transition-all duration-300 z-50 relative"
      >
...
export const metadata: Metadata = {
  title: 'Hải Lam Engineering & Construction',
...
echo "All files have been successfully created/updated."
